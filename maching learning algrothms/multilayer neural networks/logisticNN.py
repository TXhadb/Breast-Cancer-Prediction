...
def logisticNN(train_file_name, test_file_name):
    ...
    for i in range(100):
        for n in range(N):
            for l in range(L + 1):
                fx[l] = f[l](X.iloc[[n]].T)
                if l > 0:
                    df_dWf[l] = ((np.eye(len(W[l])) - logistic(W[l] @ fx[l - 1]) @ np.ones((1, len(W[l])))) @ np.diagflat(logistic(W[l] @ fx[l - 1]).values))
    ...
    print('Training time for logisticNN over 100 iterations is ' + str(time.time() - start) + ' seconds')
    ...
    res.to_csv('logisticNN-predictions.csv', index = False)

if __name__ == '__main__':
    logisticNN(sys.argv[1], sys.argv[2])
